    ...
    try:
        ...
        # Vérifiez si l'utilisateur est un étudiant
            ...
            forum = forum_models.Sujet.objects.filter(
                cours_chapitre_classe=classe
            )[:5]

            forum_count = forum.count()

            # Préparez les données pour le tableau de bord des étudiants
            datas = {
                'cours': cours,
                'forum': forum,
        ...
        # Si l'utilisateur n'a pas de rôle, redirigez vers une page par défaut
        else:
            return redirect('login')

    except Exception as e:
        print(f"Erreur dans la vue index : {e}")
        # Redirigez vers une page d'erreur ou une page par défaut
        return redirect('login')
